...
const BlocLines: FC<Props> = ({ lines, unSelect, onSelect, selection, onNoteSelect = undefined }) => {
  ...
  const sortIcon = (fieldName: ExtraitField) => {
    return fieldName === sortField ? (
      <div style={{ float: 'right' }}>{sortOrder === -1 ? <BiSolidDownArrow /> : <BiSolidUpArrow />}</div>
    ) : (
  ...
  const [sortField, setSortField] = useState<ExtraitField>('date');
  ...
  return (
    colors && (
      <table className="extraits">
        ...
        <tbody>
          {lines
            .sort((lineA: ExtraitLine, lineB: ExtraitLine) =>
            ...
            .map((item) => {
              ...
              return (
                <tr
                  key={item.id}
                  className={getLineClassName(item.id)}
                ...
                >
                  ...
                  <td>
                    ...
                    <div
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        onNoteSelect && onNoteSelect(item.id);
                      }}
                    >
