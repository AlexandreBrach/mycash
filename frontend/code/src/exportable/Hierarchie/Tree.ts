 ...
 * @returns
 */
export const treeFlatReduce = <T>(
  ref: Array<Tree<any>>,
  flat: FlatTree<T>,
  reducer: TreeReducerMethod<T>,
  initialValue: T,
): FlatTree<T> => {
    ...
    if (itemRef.children.length !== 0) {
      ...
      if (reducedChildren.length !== 0) {
        ...
        result.push({
          id: itemRef.id,
          data: result
            .filter((r) => childrenIds.indexOf(r.id) !== -1)
            .map((t) => t.data)
...
/**
 * Return a new tree where derivative was recursively applied from top to leafs
 *
 * @param tree
 * @param reducer
 * @param initialValue
