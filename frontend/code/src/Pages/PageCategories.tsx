...
const Categories: FC = () => {
  ...
  useEffect(() => {
    const selectedColor: { color: string } | undefined = selected
      ? treeFindById(state.categoryColors, selected)
      : undefined;
  ...
  const content = (
    <div id="page-categories">
      <div id="category-list" className="section no-background">
        <input type="button" value="Créer une catégorie" onClick={handleClickCreateButton} />
        <br />
        <br />
        ...
        <div className="section" id="category-edition">
          ...
          <div id="category-edition-control">
            ...
            <div id="move-action">
              ...
              <CategoryFormElement value={newAncestor} onChange={(e) => setNewAncestor(e)} zeroLabel="--racine--" />
              &nbsp;
              <button onClick={(e) => moveCategoryIn(selected, newAncestor)}>Déplacer</button>
            </div>
          </div>
        </div>
      ) : (
