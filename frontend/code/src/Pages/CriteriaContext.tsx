  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (availablesMonths[0]) {
        dispatch({
          type: 'setCriteria',
          data: { month: availablesMonths[availablesMonths.length === 0 ? 0 : 1] },
        });
      }
    };
    if (appState.availableMonths.length !== 0) {
      dispatch({
        ...
        data: appState.availableMonths[appState.availableMonths.length === 0 ? 0 : 1],
      });
    }
    void run();
