...
const PageExtraits: FC<Props> = () => {
  ...
  const content = (
    <div id="page-extraits">
      ...
      {numberSelectedLines === 0 || (
        <>
          <div id="summary">
            <strong> {numberSelectedLines} </strong> lignes sélectionnées. Total:{' '}
            <strong>{formatService.renderMontant(totalSelectedLines)} &euro;</strong>
      ...
      <ReactModal
        ...
        className="modal"
      >
        <EditNote line={noteSelection} onProcess={updateNote} abort={() => setNoteSelection(undefined)} />
      </ReactModal>
    </div>
  );
